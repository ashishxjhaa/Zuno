...
// Orbit rings use a full hue wheel with a stroke gradient
...
export interface DotRender {
  ...
  r: number
  opacity: number
  // Optional color; default is the body color
  color?: string
  // Depth haze mix: 0 fades into the background, 1 is full body color
  depth?: number
  // Optional non-circle shape in ball-radius units; when set, r is unused
  d?: string
  // Rotation applied to path d, in degrees
  rot?: number
}

// State-declared arc geometry in ball-radius units; engine scales it
...
export interface ArcRender {
  id: string
  // Part drawn in front of the body
  front: string
  // Part drawn behind the body
  back: string
  width: number
  opacity: number
...
export interface ArcSeed {
  ...
  a: number
  // Flattening b/a (orbits seen edge-on)
  k: number
  // Tilt of the major axis on screen, in radians
  tilt: number
  // Revolutions per second
  speed: number
  phase: number
  // Fraction of the orbit that is actually drawn
  sweep: number
  hue: number
  hueSpan: number
  width: number
...
// Project a tilted 3D circle; split front/back for depth
...
// Six orbit rings sized from the reference video
...
// Nested arcs that sweep before the orbits
...
// Measured x positions for the three dots; y is 0
export const DOT_X = [-0.557, -0.013, 0.532] as const
export const DOT_R = 0.165
export const DOT_PEAK = 1.18

// Particles

const P_RNG = createRng(0xbeef)

// Five particles; spawn every 0.2s; each lives 0.55s
...
// Burst particles spiral inward and pass behind the core
...
// Comet tip stays centered; the trail orbits it
...
// Comet tip radius (measured 0.129)
export const COMET_DOT = 0.129

// Notification badge

// Blue color sampled from the reference
export const NOTIF_BLUE = '#2496e8'
// Badge sits on the rim at -42 degrees
export const NOTIF_ANGLE = -42
export const NOTIF_DIST = 1.003
// Rest radius; pop peaks 14% above
export const NOTIF_R = 0.15
export const NOTIF_POP = 1.14
// Badge cutout is a concentric disk subtracted from the body
